...
| **Vilbli ↔ Min Veg table** | **Required** after production relay: (1) per-county Vilbli VG2 **map** pins vs Min Veg `is_active` VG2 PSA; (2) north `{18,55,56}` OOC pins ↔ continuations; (3) **CRITICAL — always** run I-1…I-3 **structure-only** check (merged/`html_stage_block` vs map tilbud) and **explicitly highlight** every hit as Vilbli «ложь» even when Min Veg correctly omitted them (report «none» when zero). Diffs explained or fixed via pipeline — not manual PSA. See `.cursor/rules/flag-vilbli-display-untruth.mdc`. |
...
| 8 | **Vilbli ↔ Min Veg county table PASS** (local VG2 map + north continuations + **structure-only / Vilbli «ложь» callout always**; unexplained DIFF = not closed) |
| 9 | E2E PASS |
