...
| **Status** | **PROPOSED** — owner gate for first row-level §4 closure (reference county `56`) |
| **Closure label** | `PHASE_4_LOSA_SECTION_4_ALTA_PILOT_ROW_CLOSURE_PENDING` |
| **Date (UTC)** | 2026-05-29 |
...
| Alta SNIPPET_ONLY | `provider_school`, `delivery_municipality` |
...
| `provider_school` | `snippet_only` (`T2_SCHOOL_NORDKAPP_VGS`) | **CONFIRMED** at **row** scope (Nordkapp provider for Alta delivery) |
...
|---|----------|----------|-----------|
| 1 | Bounded **CONFIRMED** promotion — Nordkapp `provider_school` | New owner gate + owner-held JSON | **1** CONFIRMED row |
...
|---|----------|------------------------|
| P4LS4A0 | Adopt Alta as **first** §4 pilot row (reference `56`)? | _pending_ |
| P4LS4A1 | County Tier 1 CONFIRMED satisfies `legal_status` + `fjernundervisning_rules` at row level? | _pending_ |
| P4LS4A2 | Re-open SNIPPET_ONLY provider/delivery for bounded CONFIRMED promotion? | _pending_ |
