...
Any next checklist-priority item requires **separate read-only selection**. **Non-selected** future candidates include: diagnostics compatibility planning; FORCE RLS separate decision; parity planning; negative-test execution planning (separate gate); read-only migration/cleanup feasibility audit for MAIN clutter — each requires its **own** owner gate.
