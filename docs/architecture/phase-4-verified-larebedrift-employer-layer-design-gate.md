...
| D-2 | Source / access | **Udir NLR API (primary, pending key) → Vilbli relay (fallback) → manual seed; Brønnøysund for identity** *(amended 2026-06-25)* | Original «owner Vigo bedrift CSV» is **not viable** — Vigo bedrift is closed to non-lærebedrifter (only fagopplæringskontoret grants access). Verified live: `data-nlr.udir.no` returns **401** (needs free Udir key, requested), unlike keyless NSR; `utdanning.no/finnlarebedrift` is a private SPA API (scrape — rejected). NLR/Vilbli both source from **VIGO**. Brønnøysund (open) canonicalizes orgnr. |
| D-3 | Geography basis | **NLR coordinates + registered kommune (Brønnøysund)** *(amended 2026-06-25)* | NLR returns `Koordinat` (lat/long) + kommune directly → separate geocoding largely unnecessary. Fallback/seed rows derive county from kommunenummer (first 2 digits); coords nullable. |
