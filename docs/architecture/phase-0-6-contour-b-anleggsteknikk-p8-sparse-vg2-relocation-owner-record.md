...
| **Status** | **CHARTERED** — implementation next |
| **Date (UTC)** | 2026-07-15 |
...
| P8-7 | Sparse eligibility | **PSA-derived** at ingest/read: `national_vg2_psa_count(anleggsteknikk) ≤ 50` (current ~30–40). **No static school list.** |
...
| Piece | Path / pattern |
|-------|----------------|
| Sparse gate | `sparse-vg2-alternative-eligibility.ts` (PSA count query) |
| Geography scope | `resolve-school-geography-scope.ts` (contour `C-VGS`, layer primary/alternative, north zone) |
| Truth merge | Load multi-county PSA for `anleggsteknikk-vg2-*` in alternative builder only |
| Step build | `build-anleggsteknikk-sparse-vg2-alternative-route-steps.ts` |
| Sync | Register in `sync-study-route-curated-regional-alternatives.ts` |
| Smokes | `smoke-anleggsteknikk-sparse-vg2-pilot.mjs` |
| E2E | Extend `e2e/route-anleggsteknikk-smoke.spec.ts` when fixture allows |
...
| Multi-contour U-10 | ☑ |
| Shared geography module | ☐ |
| Alternative builder + sync | ☐ |
| Smokes + QA build | ☐ |
| Prod UI sign-off | ☐ |
