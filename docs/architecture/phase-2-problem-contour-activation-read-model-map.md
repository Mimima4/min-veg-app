...
| Matching / identity–location resolution | **CASE 1:** one Vilbli row → one identity + one NSR location (allowed). **CASE 2:** `multi_avd_identity` clean matcher path (1:N linkage; 1:1 Vilbli-brand emission). | **CASE 2** weak ties / **CASE 3–4:** ambiguous / unsupported per matching spec; Phase 2 states capture unresolved/unsupported | Clean: resolved match path; Problem: explicit `identity_unresolved` / `location_unresolved` / `ambiguous_candidates` / `unsupported_losa` / `external_delivery` etc. | Per-`avd` programme publish without evidence; random campus; first-candidate for **different identities**; split slash aliases; ordinary match for LOSA | `norway-school-identity-matching-spec.md` — **Mandatory matching behavior** + **Forbidden**; `school-identity-location-resolution-phase-2-spec.md` — **§9**, county matrix **§15** |
...
|---------------------|------------------|--------------|---------|-------------------|-----------------|
| Multi-location / campus without per-campus programme evidence | **NON-CANONICAL DESCRIPTOR** (maps to **CASE 2** production policy) | `norway-school-identity-matching-spec.md` — **CASE 2**; `school-identity-location-resolution-phase-2-spec.md` — **§15** county `55` | Matcher links `avd` rows; emit/display **1:1** school-brand; do not publish per-`avd` programme without Tier 2+ proof | Problem-tail only for **per-campus programme** expansion; not for clean `multi_avd_identity` matcher ties | Per-`avd` programme options from identity alone; random campus |
...
| `location_unresolved` | **yes** | same — **§9** | Per-campus programme location not evidenced (distinct from `multi_avd_identity` linkage) | Problem-tail for per-campus publish | Per-`avd` programme publish from NSR identity alone |
| `ambiguous_candidates` | **yes** | same — **§9** | Multiple candidates without safe tie-break (**different** identities or weak fuzzy) | Problem-tail | First-candidate / sort-order pick for **matching** ambiguity |
...
| First-candidate / random-campus risk | **NON-CANONICAL DESCRIPTOR** | `norway-school-identity-matching-spec.md` — **Forbidden**; `school-identity-location-resolution-phase-2-spec.md` — **§15** | Bans random campus and sort-order **matching** / per-campus programme claims; **not** deterministic school-brand emission anchor | Abort / explicit states for true ambiguity | Per-`avd` programme publish; sort-order tie-break across **different** schools |
...
| Phase 3 / per-campus expansion | **Later** per-campus PSA when Tier 2+ evidence exists | N/A here | **Not opened** by this map (production already 1:1 school-brand for CASE 2) | `phase-2-to-phase-3-gate-criteria.md`; `phase-2-closure-criteria-checklist.md` |
