...
**Context:** Phase 2 evidence model closure, Phase 2 governance/review closure, Phase 2 production truth closure, and Phase 2 runtime/write closure are **Closed** at documentation / owner policy level only (`phase-2-evidence-model-owner-decision-record.md`, `phase-2-governance-review-owner-decision-record.md`, `phase-2-production-truth-owner-decision-record.md`, and `phase-2-runtime-write-owner-decision-record.md`, 2026-05-18). **Gate 34B / main RLS apply readiness** owner/security policy is logged (`phase-2-rls-apply-readiness-owner-decision-record.md`, 2026-05-18) with **NOT_READY_FOR_APPLY** and **EXECUTION_FORBIDDEN**. **RLS apply preconditions**, **RLS target-map**, **RLS accountability**, and **RLS snapshot requirements** owner/security policies are logged (`phase-2-rls-apply-preconditions-owner-decision-record.md`, `phase-2-rls-target-map-owner-decision-record.md`, `phase-2-rls-accountability-owner-decision-record.md`, `phase-2-rls-snapshot-requirements-owner-decision-record.md`, 2026-05-18) — do **not** make RLS apply ready; **NOT_READY_FOR_APPLY**, **EXECUTION_FORBIDDEN**, and **EXECUTION_PACKET_DRAFT_FORBIDDEN** unchanged. **Operational** production truth and **operational** runtime/write integration remain **Not closed** / **Blocked / not approved**. After this snapshot requirements record, the next checklist-priority item requires **separate read-only selection**; SQL, Supabase connect, live snapshot collection, and apply remain **forbidden**.
...
**Explicitly not closed by RLS readiness, preconditions, target-map, accountability, or snapshot-requirements policy records:** Gate 34B execution; staging RLS apply; main / owner-used RLS policy apply; production RLS apply; SQL; Supabase apply; Supabase connect for snapshot collection (unless a **future** gate approves); live per-target RLS snapshot **evidence**; owner-held target snapshot capture values; execution packet approval or draft; cleanup/migration/delete or new Supabase project; exact human identities for executor/approver/rollback (owner-held unless later approved for git); negative-test plan finalization; negative tests passing on real target; diagnostics post-RLS compatibility pass; FORCE RLS decision; parity evidence (staging → main); per-target operational good-restored-state values; rollback SQL artifact; operational production truth (populated rows, production resolution loop on main); operational runtime/write implementation; Phase 2 observation/candidate/decision/publication row writes; PSA publication; PSA materialization; Route Engine consumption; **active** operator/admin workflow implementation; Phase 3; Phase 4 LOSA implementation; helper/pipeline/readiness integration.
