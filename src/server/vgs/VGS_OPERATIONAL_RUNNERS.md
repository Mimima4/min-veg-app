...
| **County-local primary route** | **No patching.** Primary route in home fylke only when the full required school chain (per `vgs-path-definitions.mjs` `requiredForWrite`) exists in **county-scoped PSA**. Missing local VG2 (or any required stage) → **no primary route** — not VG1-only, not cross-fylke schools in home PSA. Neighboring-fylke / Vilbli home-listed continuations → **`alternative_routes[]` only** via P-7 (**Vilbli home list ∩ NSR ∩ destination PSA ∩ public** — no adjacency PSA dump). Charter: `docs/architecture/phase-4-county-local-primary-route-completeness-owner-policy.md` + `docs/architecture/phase-4-vilbli-home-vg2-continuation-overlay-owner-record.md`. **Do not** relax pipeline `ABORT` on missing required Vilbli stages into partial PSA writes. |
