...
export function resolvePathVariantSourceUrl(
  ...
  if (profession === "murer") {
    const chainUrl = urls.find(
      (url) =>
        /bygg-og-anleggsteknikk-skoler-og-laerebedrifter/i.test(url) &&
        /v\.babmo2|v\.babat1/i.test(url)
    );
    if (chainUrl) return chainUrl;
  }

  if (profession === "anleggsgartner") {
    const chainUrl = urls.find(
      (url) =>
        /bygg-og-anleggsteknikk-skoler-og-laerebedrifter/i.test(url) &&
        /v\.baang2|v\.babat1/i.test(url)
    );
    if (chainUrl) return chainUrl;
  }
